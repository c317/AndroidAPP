...
public class MessageService extends Service {
				...
				while (true) {
					if (netStatus.getNetWorkStatus()) {
						try {// 睡眠一段时间
							...
							frequentQuery(MyConfig.MODULEID_NOTICE,lastTimeOfNotice,1);
							lastTimeOfHotNews = dataBaseHelper.getLastTimeOfHotNews();
							Log.i(TAG, "lastTimeOfHotNews:"+lastTimeOfHotNews);
							frequentQuery(MyConfig.MODULEID_HOTNEWS,lastTimeOfHotNews,-1);
							//lastTimeOfGroupNews = dataBaseHelper.getLastTimeOfGroupNews();
	 ...
	 * @param moduleId
	 * @param lastTime
	 * @param userId 为-1时表示该接口不需要userId参数
	 */
	public void frequentQuery(final int moduleId,final String lastTime,final int userId) {
	 ...
	 */
	private void analyzeJsonData(int moduleId,JSONObject obj) throws JSONException{
		...
		case MyConfig.MODULEID_NOTICE:
			...
			for (int i = 0; i < newsJsonArray.length(); i++) {
				...
				noticeDTO.setContent(newsObject.getString(MyConfig.NEWS_CONTENT));
				noticeDTO.setSource(newsObject.getString(MyConfig.NEWS_SOURCE));
				noticeDTO.setPublisher(newsObject.getString(MyConfig.NEWS_PUBLISHER));
		...
		case MyConfig.MODULEID_HOTNEWS:
			...
			for (int i = 0; i < newsJsonArray.length(); i++) {
				...
				hotNewsDTO.setContent(newsObject.getString(MyConfig.NEWS_CONTENT));
				hotNewsDTO.setSource(newsObject.getString(MyConfig.NEWS_SOURCE));
				hotNewsDTO.setLastTime(newsObject.getString(MyConfig.NEWS_LASTTIME));
				hotNewsDTO.setPubTime(newsObject.getString(MyConfig.NEWS_PUBTIME));
