...
public class BacklogReplyDetail {
	...
	public BacklogReplyDetail(final String userId, final String token,
						...
						try {
							...
							case MyConfig.RESULT_STATUS_SUCCESS:
								if (successCallback != null) {
									...
									List<Affairs> officeReplyContent = new ArrayList<Affairs>();
									...
									for (int i = 0; i < affairsJsonArray.length(); i++) {
										...
										officeReplyContent.add(as);
									}

									successCallback.onSuccess(officeReplyContent);
								}
								break;
		...
		stringRequest.setTag("getReplyAffairsDetailpost");
