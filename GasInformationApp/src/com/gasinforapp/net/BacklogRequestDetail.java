...
public class BacklogRequestDetail {
	/**
	 * 连接到办公请求详情
 	 * @param userId
	 *            当前用户职工编号
	...
	public BacklogRequestDetail(final String userId, final String token,
		...
		StringRequest stringRequest = new StringRequest(Request.Method.POST,
				MyConfig.SERVER_URL_OA + MyConfig.ACTION_CHECKOFFICEREQUESTDETAIL,
						...
						try {
							...
							case MyConfig.RESULT_STATUS_SUCCESS:
								if (successCallback != null) {
									...
									List<Affairs> officeRequestContent = new ArrayList<Affairs>();
									...
									for (int i = 0; i < affairsJsonArray.length(); i++) {
										...
										officeRequestContent.add(as);
									}

									successCallback.onSuccess(officeRequestContent);
								}
								break;
		...
		stringRequest.setTag("getRequestAffairsDetailpost");
